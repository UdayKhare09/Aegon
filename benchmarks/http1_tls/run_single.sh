#!/bin/bash
# benchmarks/http1_tls/run_single.sh — Run exactly ONE HTTP/1.1 TLS benchmark with strict CPU affinity
set -e
...
echo "=============================================================================="
echo " Framework:   ${FRAMEWORK} (HTTP/1.1 TLS)"
...
cleanup() {
    ...
    if [ "$FRAMEWORK" = "fiber" ]; then
        pkill -f "benchmarks/http1_tls/servers/fiber/bench_fiber" 2>/dev/null || true
    fi
}

...
int main(int argc, char* argv[]) {
    ...
    std::string cert_file = "benchmarks/http1_tls/certs/server.crt";
    std::string key_file = "benchmarks/http1_tls/certs/server.key";
    if (argc > 3) cert_file = argv[3];
    if (argc > 4) key_file = argv[4];

    server.enable_tls(cert_file, key_file);
    server.enable_http3(false);
    server.listen(port);
    std::cout << "Aegon TLS listening on https://0.0.0.0:" << port << " with " << threads << " worker threads" << std::endl;
    server.run(threads);
    return 0;
}

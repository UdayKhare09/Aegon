...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Drogon TLS listening on https://0.0.0.0:" << port << " with " << threads << " threads" << std::endl;
    app().setLogPath("")
         .setLogLevel(trantor::Logger::kWarn)
         .addListener("0.0.0.0", port, true, "benchmarks/http1_tls/certs/server.crt", "benchmarks/http1_tls/certs/server.key")

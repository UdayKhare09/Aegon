...
FRAMEWORKS = {
    "aegon": {
        "port": 18080,
        "cmd": ["benchmarks/http1_tls/build/aegon/bench_aegon", "2", "18080"],
    },
    "actix": {
        "port": 18081,
        "cmd": ["benchmarks/http1_tls/servers/actix/target/release/bench_actix", "2", "18081"],
    },
    "fiber": {
        "port": 18082,
        "cmd": ["benchmarks/http1_tls/servers/fiber/bench_fiber", "2", "18082"],
    },
    "drogon": {
        "port": 18083,
        "cmd": ["benchmarks/http1_tls/build/drogon/bench_drogon", "2", "18083"],
    },
    "swerver": {
        "port": 18084,
        "cmd": ["benchmarks/http1_tls/servers/swerver/zig-out/bin/bench_swerver", "2", "18084"],
    },
}
...
def run_benchmark_triplicate(framework, endpoint, duration="10s", concurrency="100"):
    ...
    print("=" * 78)
    print(f" Triplicate Benchmark: {framework.upper()} (TLS) on {url_path} (3 runs x {duration})")
    ...
    try:
        ...
        print("-" * 78)
        print(f" ⭐ {framework.upper()} (TLS) 3-RUN AVERAGE on {url_path}:")
        ...
        try:
            ...
            proc.kill()
        if framework == "fiber":
            subprocess.run(["pkill", "-f", "benchmarks/http1_tls/servers/fiber/bench_fiber"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        time.sleep(1)

if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 run_triplicate.py <aegon|actix|fiber|drogon|swerver> <plaintext|json|user_post> [duration] [concurrency]")
        sys.exit(1)
